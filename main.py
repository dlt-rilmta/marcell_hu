...
if __name__ == '__main__':
    ...
    argparser = parser_skeleton(description='mtsv -- Marcell with e-magyar')
    ...
    if len(opts.task) > 0:
        ...
        app = pipeline_rest_api(name='marcell-with-emagyar', available_tools=tools, presets=presets,
                                conll_comments=conll_comments, singleton_store=singleton_store,
                                form_title='mtsv -- Marcell with e-magyar',
                                doc_link='https://github.com/dlt-rilmta/marcell_hu')
        app.run(debug=True)
    else:
